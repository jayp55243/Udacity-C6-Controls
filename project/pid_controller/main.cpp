...
int main ()
{
  ...
  **/
	PID pid_steer = PID();
  	//Trail 1: First tune without collision
  	//pid_steer.Init(0.29, 0.001, 0.7, 1.2, -1.2);
  
  	//Trail 2: Fine Tuning from Trial 1
  	//pid_steer.Init(0.2, 0.001, 0.8, 1.2, -1.2);
  
  	//Trial 3: Change to the error function added lateral position error alongside yaw
  	pid_steer.Init(0.2, 0.0011, 0.8, 1.2, -1.2);
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          **/
          ...
          // Find the closest point in planner to the current state of the vehicle
          for(int i=0; i<x_points.size(); ++i){
            ...
            //Establish initial min distance
            if(i == 0){
              dist_min = distance;
            }
            else{
              ...
              if(distance < dist_min){
                // Set id closest and reset min distance
                id_closest = i;
                dist_min = distance;
              }
            }
          ...
          // Yaw angle would be a better metrics to use for this controller. So angle_between_points is used for vehicle yaw and compared with path planner yaw to get error. 
           error_steer = yaw - angle_between_points(x_position, y_position, x_points[id_closest], y_points[id_closest]) + y_position - y_points[id_closest] ;
          ...
          **/
          ...
          error_throttle = velocity - v_points[id_closest];
